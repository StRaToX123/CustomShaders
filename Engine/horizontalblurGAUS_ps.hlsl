...
// Pixel Shader
////////////////////////////////////////////////////////////////////////////////
Output HorizontalBlurPixelShader(PixelInputType input)
{
	Output output;
	float col_main = 0;
	...
	for(float index = -1.0f; index > (-1.0f * Blur_Sample_Count_Halfed); index--)
	{
		...
		offset = index * texelSizeX * Blur_Size * aspectRatioInv;
		//get uv coordinate of sample
		uv = input.tex.x + offset;
		// sample the offseted pixel's MatID
		sampledValue = textureMatID.SampleLevel(SampleType_MatID, float2(uv, input.tex.y), 0).x; 
		// LATENCY HIDING
		...
		uv = shaderTexture.Sample(SampleType_MatID, float2(uv, input.tex.y)).x;
		...
		offset = uv * gausSecondPart;
		col_secondary += offset;
		offset = offset * sameMatID;
		col_main += offset;
	}
